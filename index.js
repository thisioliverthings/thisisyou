...
async function sendAnimeResponse(chatId, animeList) {
...
`;
    ...
    const newsButton = { text: 'آخر الأخبار', callback_data: `fetch_news_${animeId}` };
    const episodesButton = { text: 'الحلقات', callback_data: `fetch_episodes_${animeId}` };
    const fullDescriptionButton = { text: 'الوصف كامل', callback_data: `full_description_${animeId}` };
...
bot.on('message', async (msg) => {
    ...
    if (['مرحبا', 'مساعدة', '/start', '/help'].includes(text)) {
        bot.sendMessage(chatId, messages.welcome, { parse_mode: 'HTML' }); // إرسال رسالة الترحيب
    } else if (text.startsWith('بحث')) {
        const query = text.split(' ').slice(1).join(' '); // استخراج اسم الأنمي من الرسالة
        if (!query) {
            return bot.sendMessage(chatId, messages.inputPrompt, { parse_mode: 'HTML' }); // تذكير المستخدم بإدخال اسم الأنمي
        }

        try {
            const animeList = await searchAnime(query); // البحث عن الأنمي باستخدام الدالة
            if (!animeList.length) {
                return bot.sendMessage(chatId, messages.noResults, { parse_mode: 'HTML' }); // إبلاغ المستخدم بعدم العثور على النتائج
            }
            await sendAnimeResponse(chatId, animeList); // إرسال رد الأنمي مع الأزرار
        } catch (error) {
            bot.sendMessage(chatId, messages.errorFetching, { parse_mode: 'HTML' }); // إبلاغ المستخدم بحدوث خطأ
        }
    } else {
        // توجيه المستخدم إلى المساعدة بدل إظهار رسائل خطأ متكررة
        bot.sendMessage(chatId, messages.unknownCommand, {
            parse_mode: 'HTML',
        });
    }
});
